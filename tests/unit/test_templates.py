...
def test_floor_length_garments_hang_from_the_hips(template_catalog: TemplateCatalog):
    ...
    Garments with legs of their own — trousers, a catsuit, tights — follow the legs.
    """
    for template in template_catalog:
        if template.hem not in {"floor", "ankle"} or template.category in {"trousers", "jumpsuit", "legwear"}:
            continue
